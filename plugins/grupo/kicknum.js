const handler = async (m, {conn, args, groupMetadata, participants, usedPrefix, command, isBotAdmin, isSuperAdmin}) => {
  if (!args[0]) return conn.reply(m.chat, `${emoji} Ingrese Algun Prefijo De Un Pais para ejecutar el comando.`, m, fake);
  if (isNaN(args[0])) return conn.reply(m.chat, `${emoji} Ingrese Algun Prefijo De Un Pais\nEjemplo: ${usedPrefix + command} 201`, m, fake);
  ...
  switch (command) {
    case 'listanum': case 'listnum':
      ...
      break;
    case 'kicknum':
      if (!bot.restrict) return conn.reply(m.chat, `${emoji} ¡Este Comando Esta Desabilitado Por El Propietario Del Bot!.`, m, racanl);
      if (!isBotAdmin) return m.reply(`${emoji2} El bot no es admin.`);
      await conn.reply(m.chat, `♻️ Iniciando eliminación....`, m, fake);
      ...
      for (const user of users) {
        ...
        if (user !== ownerGroup + '@s.whatsapp.net' && user !== global.conn.user.jid && user !== global.owner + '@s.whatsapp.net' && user.startsWith(lol || lol) && user !== isSuperAdmin && isBotAdmin && bot.restrict) {
          ...
          await delay(10000);
        } else return m.reply(m.chat, `${msm} Ocurrió un error.`, m, fake);
      }
      break;
  }
};
handler.command = handler.help = ['kicknum', 'listnum', 'listanum'];
handler.tags = ['grupo']
handler.group = true;
